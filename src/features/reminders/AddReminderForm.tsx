...
export const AddReminderForm = () => {
  ...
  const userRole = useAppSelector(selectUserRole); // ★★★★★ userRoleを直接取得 ★★★★★
  const currentServer = servers.find(s => s.id === serverId);

  const { canCreate } = useServerPermission(serverId);

  // ★★★★★ ここからが修正箇所です ★★★★★
  // サーバー情報またはユーザー権限が読み込み中の場合
  ...
  if (!canCreate) {
    return (
      <Box sx={{ p: 4, textAlign: 'center' }}>
        <Typography variant="h5" gutterBottom>アクセス権がありません</Typography>
        <Typography color="text.secondary" paragraph>
          リマインダーを新規作成できるのは、サーバーの管理者権限を持つオーナーまたはテスターのみです。
        </Typography>
        <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate(`/servers/${serverId}`)}>
          リマインダー一覧に戻る
        </Button>
      </Box>
    );
  ...
  if (isHitServer) {
    if (typeParam === 'boss') return <AddBossReminderForm />;
    else if (typeParam === 'hydra') return <AddHydraReminderForm />;
    else return <ReminderForm mode="add" />;
  }
